...
import { UpdateAndNewClient } from '../modals/newClient'
...
export const ClientInformation = ({ client, refetch }: Props) => {
  ...
  return (
    <>
      <article className='flex flex-col p-4 gap-8 bg-white rounded-xl mt-10 shadow-md w-1/4'>
        ...
        <button onClick={() => toggleModal(true)} className='p-2 bg-gray-600 rounded text-white'>Editar</button>
      </article>

      {showModal && <UpdateAndNewClient onUpdate={refetch} setOpen={toggleModal} defaultValue={client} />}
    </>
  )
}